	...
	def post(self, request, *args, **kwargs):
		...
		if form.is_valid():
			...
			try:
				...
				pass
		return render(request, 'ventas/consulta_pedido_venta.html',
					  {'form': form, 'listado_pedido_venta': listado_pedido})
...
def modificar_pedido(request):
	...
	if request.GET.get('idPedidoVenta', None):
		...
		if pedidoVenta.estadoPedidoVenta.codigo != "01":
			messages.warning(request,
							 'El pedido no se puede modificar porque esta con estado %s' % pedidoVenta.estadoPedidoVenta.descripcion)
			pedidoVenta = None
		else:
	...
	return render(request, "ventas/modificar_pedido.html",
				  {'pedidoVenta': pedidoVenta, 'listado_motivo_cancelacion': listado_motivo_cancelacion})
	...
	if idPedidoVenta and idSaldoInventario and cantidad and costoTotal:
		try:
			...
			pedidoVentaPosicion = PedidoVentaPosicion(pedidoVenta=pedidoVenta, producto=saldoInventario.producto,
													  proveedor=saldoInventario.proveedor, cantidad=cantidad,
													  costoTotal=costoTotal)
			# Se valida que no exista el producto en la lista con el mismo proveedor
			if PedidoVentaPosicion.objects.filter(pedidoVenta=pedidoVenta, producto=saldoInventario.producto,
												  proveedor=saldoInventario.proveedor).exists():
				respuesta['resultado'] = "El pedido ya tiene el producto %s del proveedor %s" % (
				saldoInventario.producto, saldoInventario.proveedor)
			else:
...
def modificar_pedido_posicion(request, idPedidoVentaPosicion):
	...
	respuesta = {'resultado': 'True', 'cantidad': cantidad, 'costoTotal': costoTotal, 'cancelado': cancelado,
				 'motivoCancelacion': idMotivoCancelacion}
	try:
		...
		if cancelado is not None:
			...
			if idMotivoCancelacion is not None:
				pedidoVentaPosicion.motivoCancelacionPedidoVenta = MotivoCancelacionPedidoVenta.objects.get(
					pk=idMotivoCancelacion)
		elif cantidad is not None and costoTotal is not None:
...
def pedido_venta_json(request):
	...
	datos = json.loads(serializers.serialize('json', PedidoVenta.objects.filter(estadoPedidoVenta__codigo='01'),
											 fields=('numeroPedido', 'cliente', 'fecha', 'estadoPedidoVenta'),
...
def pedido_venta_posicion_json(request, idPedidoVenta):
	...
	datos = json.loads(serializers.serialize('json', PedidoVentaPosicion.objects.filter(pedidoVenta=idPedidoVenta),
											 fields=('producto', 'proveedor', 'cantidad', 'costoTotal'),
											 use_natural_foreign_keys=True))
	# creamos el diccionaro para guardar los datos
	# al diccionario le agregamos u
	dic = {'data': datos}
...
def busqueda_pedido_venta_modal(request):
	...
	columns = {'Numero Pedido': 'fields.numeroPedido', 'Cliente': 'fields.cliente', 'Fecha': 'fields.fecha',
			   'Estado': 'fields.estadoPedidoVenta'}
	...
	return render(request, "base/tabla-dinamica-modal.html", {'title': 'Busqueda de Pedido Venta', 'url': url,
															  'columns': columns, 'objectPk': objectPk,
...
def pedido_venta_compra_json(request, idPedidoVentaPosicion):
	...
	listado_posicion = list(({'pk': p.pk, 'numeroPedido': p.pedidoCompraPosicion.pedidoCompra.numeroPedido,
							  ...
							  'cantidad': p.cantidad} for p in
							 PosicionVentaCompra.objects.filter(pedidoVentaPosicion=idPedidoVentaPosicion)))
	# creamos el diccionaro para guardar los datos
	# al diccionario le agregamos u
