...
class SASerializer:
	...
	def updateSaldoInventario(self):
		if self.saldoInventario is not None:
			# si hay valor, se actualizan los precios
			# de lo contrario se pone el estado del saldo inventario False
			if self.valor > 0:
				...
				self.saldoInventario.estado = False
...
class ActualizacionPrecio:
	def __init__(self,archivoModificacionPrecio):
		...
		# almacena los Saldo Inventarios que no se encontrarón
		# en los listados de sincronización
		self.listado_no_encontrado = []
	...
	def make_actualizacion(self):
		...
		# se obtienen los SI que no fueron encontrados en el listado
		for saldo in SaldoInventario.objects.filter(proveedor_id=self.proveedor.pk, estado=True).exclude(
			pk__in=list(x.saldoInventario.pk for x in self.listado_pendiente_actualizar)).exclude(
			pk__in=list(x.saldoInventario.pk for x in self.listado_sin_cambio)):

			sa = SASerializer(saldo.referenciaProveedor,saldo.producto.nombre,saldo.precioVentaUnitario)
			sa.saldoInventario = saldo
			self.listado_no_encontrado.append(sa)
				...
				for row in range(0,xl_sheet.nrows):
					...
					for col_cod in columns_codigo:
						...
						if xl_sheet.cell_type(row,col_cod) is 2:
							...
							# si el campo valor es un text quiere decir que no tiene precio (Sin Stock)
							# el valor se pone en 0 para que se actualice el SA con estado=False
							if xl_sheet.cell_type(row,col_cod+ 2) is 1:
								# si existe SA con la referencia, se debe agregar para cambiar el estado
								if SaldoInventario.objects.filter(referenciaProveedor=codigo,
																  proveedor_id=self.idProveedor,estado=True).exists():
									for saldo in SaldoInventario.objects.filter(referenciaProveedor=codigo,
																  proveedor_id=self.idProveedor,estado=True):
										pofo_sin_stock = SASerializer(codigo,producto,0)
										pofo_sin_stock.saldoInventario = saldo
										self.listado_pendiente_actualizar.append(pofo_sin_stock)
								continue
