...
def lista_precios(request):
	...
	listado_saldo_inventario = SaldoInventario.objects.filter_products().order_by('producto__categoria__codigo',
																				  'producto__numeroProducto').values(*fields)
	listado_categoria = []
	codigo_categoria = "0"
	for saldo in listado_saldo_inventario:
		if len(listado_categoria) == 0 or saldo['producto__categoria__codigo'] != codigo_categoria:
			...
			if categoria.categoriaPadre:
				listado_categoria.append(
					{'categoria': "%s - %s " % (categoria.categoriaPadre.descripcion, categoria.descripcion),
					 'listado_saldo_inventario': []})
			else:
