...
class producto_categoria(ListAPIView):
	...
	def get_queryset(self, *args, **kwargs):
		...
		filter_Q = None
		if categoria:
			filter_Q = Q(producto__categoria__codigo=categoria) | Q(
				producto__categoria__categoriaPadre__codigo=categoria)
		else:
...
class oferta(ListAPIView):
	...
	def get_queryset(self, *args, **kwargs):
		...
		listado_promociones = Promocion.objects.only('saldoInventario_id').filter(fechaFin__isnull=True, estado=True,
																				  saldoInventario__precioOferta__gt=0,
																				  saldoInventario__estado=True).order_by(
			'-fechaInicio')
...
class search_producto(ListAPIView):
	...
	def get_queryset(self, *args, **kwargs):
		...
		filter_Q = Q()
		listado_categorias = Categoria.objects.filter(
			reduce(operator.or_, (Q(descripcion__icontains=x) for x in list_filtro)))
...
def saldo_inventario_simple(request):
	...
	listado = (SaldoInventarioListSimpleSerializer(instance=p).data for p in
			   SaldoInventario.objects.filter(pk__in=list(d.get("idSaldoInventario") for d in data)))
...
class oferta_index(ListAPIView):
	...
	def get_queryset(self, *args, **kwargs):
		listado_promociones = Promocion.objects.filter_promocion(fechaFin__isnull=True,
																 estado=True,
																 saldoInventario__precioOferta__gt=0,
																 saldoInventario__estado=True).only(
			'saldoInventario_id').order_by('-fechaInicio')[:10]
		return SaldoInventario.objects.filter_products(
			pk__in=list(p.saldoInventario_id for p in listado_promociones)).order_by('-estado')
	...
	def get_queryset(self, *args, **kwargs):
		...
		for idProducto in review:
			if SaldoInventario.objects.filter_products(estado=True, producto=idProducto).exists():
				listado_idSaldoInventario.append(
					SaldoInventario.objects.filter_products(estado=True, producto=idProducto).only('pk').first().pk)
			if len(listado_idSaldoInventario) >= top:
				break
		return SaldoInventario.objects.filter_products(estado=True, pk__in=listado_idSaldoInventario).order_by(
			'-estado')
