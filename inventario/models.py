...
class Categoria(models.Model):
	...
	categoriaPadre = models.ForeignKey("Categoria", db_column='idCategoriaPadre', blank=True, null=True,
									   on_delete=models.CASCADE, verbose_name='Categoría Padre')
...
# almacena las plataformas de los software (videojuegos)
...
class SaldoInventarioManager(models.Manager):
	...
	def filter_products(self, *args, **kwargs):
		...
		for sa in listado_saldo_inventario:
			...
			if listado_saldo_inventario.filter(
				producto_id=sa.producto_id).count() > 1 and not sa.pk in listado_pk_exclude:
				...
				if not sa.estado or listado_saldo_inventario.filter(producto_id=sa.producto_id,
					...
					for exclude in listado_saldo_inventario.filter(producto_id=sa.producto_id).exclude(
						pk=sa.pk).values_list('pk', flat=True):
...
class SaldoInventario(models.Model):
	...
	def save(self, *args, **kwargs):
		...
		if self.idSaldoInventario is None and self.precioVentaUnitario <= 0:
			...
			if self.cantidad > 0:
				self.precioVentaUnitario = Decimal(self.costoTotal / self.cantidad) * (
				1 + porcentajeGanancia.porcentaje / 100)
			else:
				self.costoTotal = 0
...
class Promocion(models.Model):
	...
	def save(self, *args, **kwargs):
		...
		if self.estado is True:
			try:
				listadoPromocion = Promocion.objects.filter(saldoInventario=self.saldoInventario, estado=True).exclude(
					pk=self.pk)
