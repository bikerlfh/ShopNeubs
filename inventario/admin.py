...
class ProductoCategoriaListFilter(admin.SimpleListFilter):
	...
	def lookups(self, request, model_admin):
		...
		return tuple((c.pk, '%s - %s (%s)' % (c.codigo, c.descripcion, (
		Producto.objects.filter(Q(categoria=c.pk) | Q(categoria__categoriaPadre=c.pk)).count()))) for c in
...
class CategoriaPadreListFilter(admin.SimpleListFilter):
	...
	def lookups(self, request, model_admin):
		return tuple(
			(c.pk, '%s - %s (%s)' % (c.codigo, c.descripcion, (Categoria.objects.filter(categoriaPadre=c.pk).count())))
			for c in Categoria.objects.filter(categoriaPadre=None).order_by('codigo'))
...
class ProductoAdmin(admin.ModelAdmin):
	fieldsets = [
		...
		('Plataformas (Solo Software)', {'fields': ['plataformas']})
	]
...
class CategoriaSaldoInventarioListFilter(admin.SimpleListFilter):
	...
	# el filtro son las categorias
	def lookups(self, request, model_admin):
		return tuple((c.pk, '%s - %s (%s)' % (c.codigo, c.descripcion, (SaldoInventario.objects.filter(
			Q(producto__categoria=c.pk) | Q(producto__categoria__categoriaPadre=c.pk)).count()))) for c in
					 Categoria.objects.all().order_by('codigo'))
	...
	def queryset(self, request, queryset):
		...
		return queryset.filter(
			Q(producto__categoria=self.value()) | Q(producto__categoria__categoriaPadre=self.value()))
...
class SaldoInventarioAdmin(admin.ModelAdmin):
	...
	list_display = ['producto', 'proveedor', 'referenciaProveedor', 'cantidad', 'costoTotal', 'precioCompraUnitario',
					'precioVentaUnitario', 'precioOferta', 'estado']
...
class PromocionAdmin(admin.ModelAdmin):
	...
	search_fields = ['saldoInventario__producto__numeroProducto', 'saldoInventario__producto__nombre',
					 'saldoInventario__proveedor']
