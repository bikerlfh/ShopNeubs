...
function getJSON(url,success,options=null){return $.ajax({dataType:"json",url:url,data:options!=null?options.data||null:null,method:options!=null?options.method||"GET":"GET",beforeSend:options!=null?options.beforeSend||null:null,success:success,error:function(jqXHR,estado,error){console.log(error);console.log(estado)},timeout:timeout_ajax})}
...
else $(objeto).remove()},error:function(jqXHR,estado,error){console.log(error);console.log(estado)},timeout:timeout_ajax})}
...
{console.log(estado)},timeout:timeout_ajax})}
...
function format_currency(value){return "$"+parseInt(value).toString().replace(/(\d)(?=(\d{3})+$)/g,"$1.")}(function($){$.get=function(key){key=key.replace(/[\[]/,'\\[');key=key.replace(/[\]]/,'\\]');var pattern="[\\?&]"+key+"=([^&#]*)";var regex=new RegExp(pattern);var url=unescape(window.location.href);var results=regex.exec(url);if(results===null){return null}else{return results[1]}}})(jQuery);$(document).ready(function(){$(window).scroll(function(){if($(this).scrollTop()>120)$('.to-top').fadeIn();else $('.to-top').fadeOut()});$(".navbar a, footer a[href='#myPage']").on('click',function(event){if(this.hash!==""){event.preventDefault();var hash=this.hash;$('html, body').animate({scrollTop:$(hash).offset().top},900,function(){window.location.hash=hash})}});var altoHeader=$("#navbar-principal").height();$(".container-fixed-top").css("padding-bottom",altoHeader+"px");if(screen.width<991)
{$('body').on("click","#buttonsearch",function(){$('#formsearch').slideToggle("fast",function(){$('#buttonsearch').hide("fast")});$('#searchbox').focus()});$('#searchbox').blur(function(){$('#formsearch').hide("fast");$('#buttonsearch').css("display","block")})}
else{$('#formsearch').css("display","block");$('#buttonsearch').hide("fast")};setTimeout(function(){$(".alert").fadeOut(1500)},3000)});$.ajaxPrefilter(function(options,original_Options,jqXHR){options.async=!0});(function(a){a.fn.validarCampo=function(options){switch(options.type){case 'number':characters='0123456789'
break;case 'char':characters='abcdefghijklmnñopqrstuvwxyzáéíóú '
break;default:characters="0123456789abcdefghijklmnñopqrstuvwxyzáéíóúABCDEFGHIJKLMNÑOPQRSTUVWXYZÁÉÍÓÚ "
break}
max_length=options.length||null;a(this).on({keypress:function(a){if(max_length==null||$(this).val().length<=max_length-1){var c=a.which,d=a.keyCode,e=String.fromCharCode(c).toLowerCase(),f=characters;(-1!=f.indexOf(e)||9==d||37!=c&&37==d||39==d&&39!=c||8==d||46==d&&46!=c)&&161!=c||a.preventDefault()}
else a.preventDefault()}})}})(jQuery)